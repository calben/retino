...
def produce_axon_interaction_demos_with_postsynapse_coloring():
  targets = add_jitter_to_points([np.asarray([50.0,50.0])] * 20, 30) 

  dots_origins = list(map(np.asarray, [[25.0] * 2, [30.0] * 2, [35.0] * 2, [40.0] * 2, [45.0] * 2]))
  dots_sizes = [5] * len(dots_origins)

  post_synapses = [PostSynapse(i, size=10, tectum_length=retino.TECTUM_WIDTH, tectum_width=retino.TECTUM_HEIGHT) for i in range(100)]
  ...
  for i in range(0,len(axons[0].history),10):
    ...
    plt.savefig(output_directory + "/ColoredByTarget-" + str(i) + ".png", dpi=200)
    plt.close(fig)

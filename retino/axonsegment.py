...
class AxonSegment(object):
  ...
  def __init__(self, id, parent, owner, origin=np.array([0.0,0.0]), end=np.array([0.0,0.0]), branch_number=-1, stability=5):
    self.id = id
    self.parent = parent
    self.owner = owner
    self.origin = origin
  ...
  def grow_post_synaptic_connections(self, post_synapses):
    origins = generate_random_points_along_line(self.origin, cart_to_pol(self.end - self.origin), retino.AXON_SEGMENT_SYNAPSE_POOL_COUNT, retino.AXON_SEGMENT_SYNAPSE_COUNT, retino.AXON_SEGMENT_SYNAPSE_JITTER_STD)

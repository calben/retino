...
def generate_random_points_distribution_in_space(xmin, xmax, ymin, ymax, points_count):
    points = [np.asarray([(xmax - xmin) / 2.0, (ymax - ymin) / 2.0])] * points_count
    print(points)
    points = add_jitter_to_points(points, ((xmax - xmin) + (ymax - ymin)) / 16)
    return points
...
def add_jitter_to_points(points, jitter_amount):
    points = [point + np.asarray([np.random.normal(0, jitter_amount), np.random.normal(0, jitter_amount)]) for point in
              points]
    return points
...
def image_to_activity_points(image_str, resolution=100):
    ...
    origins = list(
            zip(list(range(resolution)) * resolution,
                np.asarray([[i] * resolution for i in range(resolution)]).flatten()))
    return [origins, activities]


def generate_evenly_spaced_grid(xmin, xmax, ymin, ymax, points_count_sqrt):
    xoffset = 2 * (xmax - xmin) / points_count_sqrt ** 2
    yoffset = 2 * (ymax - ymin) / points_count_sqrt ** 2
    return np.mgrid[xmin + xoffset:xmax + xoffset:(xmax - xmin) /
                                                  points_count_sqrt,
           ymin + yoffset:ymax + yoffset:(ymax - ymin) / points_count_sqrt].reshape(2, -1).T


def generate_jittered_points_on_grid(xmin, xmax, ymin, ymax, points_count_sqrt, jitter_amount):
    grid = generate_evenly_spaced_grid(xmin, xmax, ymin, ymax, points_count_sqrt)
    return add_jitter_to_points(grid, jitter_amount)
...
def make_diagonal_activity_waves(length, wave_width, wave_offset):
    result = []


def rising_hill_function(x, k, h):
    return x ** h / (k ** h + x ** h)
